...
```
It is not converged yet. Ideally, we need to have more iterations. However, it has already been busy for the system to run with 5 iterations. 
...
#ave_icu <- rbindlist(new_list, use.names=TRUE)[,lapply(.SD, mean)] 
...
#predict the 30-day mortality of ICU patients using the training set
mortality_pre <- predict(logit, test, type = "response")
```

```{r}
#Calculate RMSE to evaluate results
library(Metrics)
RMSE_result <- 
  rmse(test$death_within_monTRUE, mortality_pre) %>%
  print()
