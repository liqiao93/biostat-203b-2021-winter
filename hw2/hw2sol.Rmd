...
admission %>% 
  ...
  scale_x_continuous(breaks = seq(0, 15, by = 1)) +   #only present the admission frequency < 15#
...
```{r}
admission %>% 
  distinct(subject_id, .keep_all = TRUE) %>%  #remove duplicates - patients with multiple visits
  ggplot(mapping = aes(x = insurance, y = ..prop.., group = 1)) +
  geom_bar(fill = c("pink", "darkolivegreen1", "lightblue")) + 
  scale_y_continuous(breaks = seq(0, 1, by = 0.1), labels = scales::percent) +
  geom_text(aes(label = scales::percent(..prop..),               
                   y= ..prop.. ), stat= "count", vjust = -.5) +   #add percentage labels to the top of bar charts
...
- **ethnicity**: Most patients are white.
```{r}
admission %>% 
  distinct(subject_id, .keep_all = TRUE) %>%  
  ggplot(mapping = aes(x = ethnicity, y = ..prop.., group = 1, stat = "count")) +
...
- **death**:
I would first check if hospital_expire_flag is consistent with death_time. check the proportion
```{r}
admission %>% 
  distinct(subject_id, .keep_all = TRUE) %>%  
  ggplot(mapping = aes(x = ethnicity, y = ..prop.., group = 1, stat = "count")) +
