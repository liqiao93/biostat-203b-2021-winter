...
**Solution**: I found that there are no duplicates based on subject_id.
``` {r}
...
- **anchor_age**: I have observed an overall pattern that the percentage of patients decreased with increasing anchor age. About 3% of patients were at age 20. An unexpected bar was observed for patients who were over 90 years old.
```{r}
patient %>%
#I observed a high peak at 0 likely due to missing values, so I only keep anchor_age that is not equal to 0#
  filter(anchor_age != 0) %>%
  ggplot(aes(x = anchor_age, y = ..prop.., group = 1)) +
