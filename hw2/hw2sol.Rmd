...
#It takes 13 seconds to read in the smaller data set#
...
Generate a subset of `labevets.csv.gz` only containing the selected items. 
```{r}
...
```
```{r}
labevents_subset <- read_csv("labevents_subset.csv.gz")
labevents_subset 
```
...
I first tried the codes below but it did not run successfully due to storage issue.  
```{r, eval = F}
...
Read the filtered subset file provided by Dr. Zhou. 
```{r}
chartevents_tble <- 
read_csv( 
"/usr/203b-data/mimic-iv-derived-data/chartevents_filtered_itemid.csv.gz") %>% 
  print(width = Inf)
```
...
ditems_tble <- read_csv(str_c(mimic_path, "/icu/d_items.csv.gz")) %>%
  print(width = Inf)
```
Items that are related to heart rate:
```{r}
item <- 'heart rate'
hr_items <- ditems_tble %>%
  filter(str_detect(label, regex(item, ignore_case = T))) %>%
  print(width = Inf)
```

Items that are related to `blood pressure mean`:
```{r}
item <- 'blood pressure mean'
systolic_items <- ditems_tble %>%
  filter(str_detect(label, regex(item, ignore_case = T))) %>%
  print(width = Inf)
```

Find the most frequent item related to systolic:
```{r, eval=F}
items <- c("heart rate", "systolic", "body temperature", 
              "SpO2", "respiratory rate")

itemids <- rep(0, length(items)) 
i <- 1
for (item in items) {
  print(str_interp("search for ${item}"))
  itemid_list <- ditems_tble %>%
    filter(str_detect(label, regex(item, ignore_case = T))) %>%
    .$itemid
...
```{r}
vitalitem <- tibble(itemid = itemids, item = items) %>%
  print(width = Inf)
```
